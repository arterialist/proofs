...
4. **A New Frontier Breakthrough:** The resolution of the small-cofactor barrier ($k=2$) via the **Dual Dirichlet Polynomial Cancellation of Proper Cofactors**, the **Dual Dirichlet Offline Power Suppression Theorem**, the **Exact Proper-Cofactor Spectral Neutralization Theorem**, and the **Weil Sign Incompatibility Theorem**, proving that the Riemann scattering multiplier $\chi(\rho)$ exactly neutralizes the raw off-line zero amplitude ($T^\varepsilon \cdot T^{-\varepsilon} = 1$), while an uncompensated off-line zero forces an asymptotic negative drift contradicting the positive arithmetic Weil margin.
