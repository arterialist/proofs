...
The [complete allocation identity](prime-allocation-critical-identity.md), with its endpoint and both density terms, is exactly
...
An unconditional bound $\psi(t)\le ct$ with fixed $1<c<2$ makes the brackets in (10) nonpositive. For an available explicit choice see [the Rosser–Schoenfeld bound used in the combined covariance theorem](combined-prime-density-covariance.md#failure-of-monotonicity-and-an-all-scale-replacement). Set
...
Quantitative PNT and partial summation give $\sum_{q\le y}\Lambda(q)/q=\log y+O(1)$; the same classical input is stated in the [harmonic quadrature theorem](harmonic-quadrature-correction.md). The first term is $\zeta(3/2)x^2\log x+O(x^2)$, and the second is $O(x^2)$ by $\sum_{q\le y}q\Lambda(q)\le y\psi(y)=O(y^2)$. Finally,
