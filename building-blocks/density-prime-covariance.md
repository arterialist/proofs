...
The scores and density term are derived from the complete measure identity in [prime allocation](prime-allocation-critical-identity.md). The certificate uses Python's standard library only. Run `python3 building-blocks/certificates/cutoff_density_monotonicity.py` from the repository root without `-O`; the monotonicity beyond the finite starting cases is the analytic induction above.
