...
This written proof refines the [complete seed's logarithmic memory](collective-successor-seed-long-memory.md) and the [positive clock's exact Laplace formula](positive-successor-seed-jump-process.md). It uses the classical Euler product and Laurent expansion at the pole $1$, with the full prime-power correction retained. There is no Lean or priority claim.
...
The [proved imaginary-axis boundary identity](positive-successor-seed-jump-process.md), equation (9), agrees with this local continuation. For 0<|t|<epsilon,
