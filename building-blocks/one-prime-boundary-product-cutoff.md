...
$$
This agrees with the orientation in [prime innovation finite band flux](prime-innovation-finite-band-flux.md) and the local factors in [the signed phase transport](signed-scattering-phase-transport.md). All proper powers remain in the absolutely convergent phase series. The unprojected cancellation is simply
$$
...
The companion proof [two history bilinear storage](two-history-bilinear-storage.md) gives, in the dyadic case,
$$
...
Use the fixed smooth flat-top multiplier and inverse kernel from [product location frequency cutoff](product-location-frequency-cutoff.md):
$$
