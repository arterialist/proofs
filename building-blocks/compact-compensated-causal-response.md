...
The arithmetic inputs are proved in the [complete memory kernel](charge-compensated-successor-memory.md) and the [original derivative response](charge-compensated-clock-response.md). We consume those identities, including every prime power and the initial interval.
