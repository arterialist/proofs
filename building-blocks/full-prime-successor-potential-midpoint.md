...
This extends the [finite-prime two-port construction](positive-primitive-two-port-drift-curvature.md). Sections 1–4 retain each actual fixed source cutoff and the original maximal multiplier domains. Sections 5–8 use local prime-kernel variation and block convolution to prove uniform bounds and joint convergence for the successor correction. The [fixed-source companion](all-prime-fixed-source-drift-curvature.md) retains the shorter finite-measure method. No sign is assigned to the full midpoint curvature.
...
The block convolution estimate is the elementary Young inequality with a two-fold support-overlap bound. It supplements the classical finite-measure argument in the fixed-source companion. The [actual finite-history example](actual-finite-history-curvature-domains.md) explains why the resulting distributional curvature cannot simply be assumed to be a nonnegative measure. These are written proofs with exact arithmetic domains and no priority claim.
