...
\tag{1}
$$
Here $D_af(x)=\sqrt a f(ax)$. The seed jump, density subtraction, prime powers and both cutoff endpoints are part of this source. The metric is $M=m(D_x)=4I-\mathcal K$, where $\mathcal Kf=\kappa*f$, $\kappa(x)=1$ for $|x|\le1$ and $\kappa(x)=|x|^{-2}$ otherwise. Its exact adjoints and single-prime domain are proved in [prime-transport-in-mixed-completion.md](../prime-distribution/prime-transport-in-mixed-completion.md).
...
\tag{11}
$$
Uniform convergence on the fixed observation window permits this finite harmonic sum and finite integral. The complete arithmetic quantities use the [harmonic-convolution normalization](../analytic-methods/harmonic-convolution.md):
...
Here $W$ is exactly the complete sum in the [actual sign criterion](../zeta-and-zeros/actual-critical-sign-criterion.md). The harmonic-convolution theorem proves its full identity and $s_{\rm crit}(t)=t^2/6+O(t)$ without discarding prime powers.
