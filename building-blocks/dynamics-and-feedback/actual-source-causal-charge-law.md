...
The causal mixed source space, complete memory $K=B*$, derivative $A=(1/2-\partial)K$, and relative charge $\delta$ are those of the [original derivative response](../factorial-and-renewal/charge-compensated-clock-response.md). The [anchoring theorem](../successor-and-division/charge-compensated-successor-memory.md#weighted-coercivity-and-the-bounded-extension) gives $\|e^{-u/2}f\|_2\le\|g\|_{\mathcal E}$ for $f(u)=e^{u/2}g(e^u)$. The actual source is $g(x)=1_{x\ge1}[\psi(x)-x]/x$, with physical representative $a(u)=e^{-u/2}[\psi(e^u)-e^u]$ and [relative charge](../successor-and-division/actual-successor-resonance-charge-cocycle.md) $\delta(a)=\kappa>0$.
...
Write $\ell=\log2$, $\widehat B(j)=\int e^{-jv}B(v)\,dv$ and $J=\widehat B(1)>0$. The [proved column identities](../factorial-and-renewal/charge-compensated-clock-response.md), with $c_j=\binom{2j}{j}/4^j$, are
...
$$
This is the [proved bounded causal action](../successor-and-division/positive-successor-seed-jump-process.md) on $\mathcal E$. Since δ is continuous on $\mathcal E_c$, the actual relative charge
$$
