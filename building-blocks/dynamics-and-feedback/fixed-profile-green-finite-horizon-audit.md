...
For a fixed prime $p$ and its fixed finite old packet, let $F_X,U_X$ be the actual finite-horizon old and delayed vectors and $F,U$ their limiting Green vectors. Write $E_X=F_X-F$ and $V_X=U_X-U$. The exact identity formalized in [FixedProfileGreenAdmissionFinite.lean](../../formalization/BuildingBlocks/FixedProfileGreenAdmissionFinite.lean) is
