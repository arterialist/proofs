...
[ChargeFrozenIntegrability](BuildingBlocks/ChargeFrozenIntegrability.lean) adds the literal frozen source's global L¹ membership and absolute convergence of every real-frequency Fourier integral, with [precise scope](frozen-source-fourier-integrability.md). [ChargeFrozenFourier](BuildingBlocks/ChargeFrozenFourier.lean) proves the [exact finite-source Fourier–Mellin identity](frozen-source-exact-fourier-mellin.md), retaining every Mangoldt power, the actual density integral and seed −1. The [uniform spectral envelope and actual logarithmic-moment application](frozen-source-spectral-logarithmic-bound.md) are compiled in `ChargeFrozenSpectralBound` and `ChargeFrozenLogMoment`, reusing the existing unconditional Chebyshev and Abel bounds. These results do not provide the original RH estimate.
