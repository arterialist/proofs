...
The [established actual tail](collective-successor-seed-long-memory.md) $F(v)\sim2/v^2$ gives
$$
...
[Dudek, Theorem 2.1](https://arxiv.org/pdf/1401.4233) and the right-continuous endpoint argument in the [zero-abscissa growth proof](zero-abscissa-scattering-growth.md). Substitution in (13) and contraction give $O_\epsilon(N^{2\Theta-1+\epsilon})$. For the reverse bound apply (27) to every exponent above the limsup. If $\Theta=1/2$, nonnegativity of the displayed logarithm supplies the lower bound zero.
