...
These are written proofs using the exact [finite geometric history law](complete-prime-history-entropy.md), ordinary Haar orthogonality and exponential-moment estimates; no Lean formalization or priority claim is made.
...
This is a statement for fixed shrinking bands after the large-$X$ limit. It does not bound coherent off-diagonal sums over an arbitrary actual observation window. The [signed divisor-band identity](signed-divisor-band-correlations.md) retains those terms explicitly.
