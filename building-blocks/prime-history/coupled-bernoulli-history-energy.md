...
The [linear Bernoulli synthesis](../successor-and-division/integer-history-low-band-pairing.md#5-the-linked-successor-jumps-turn-the-leading-chirp-into-exact-synthesis) supplies the exact successor and division formulas. The [max-kernel norm](../dynamics-and-feedback/max-sum-process-transfer.md) fixes the original metric, and the [complete process](../successor-and-division/positive-successor-seed-jump-process.md) supplies its contraction. The [rational certificate](../../certificates/heat-birth-sign-certificate.py) evaluates (5)–(7) with proved logarithm remainders and all density pieces. Bernoulli expansion and divisor regrouping are classical; the actual weighted arithmetic form and its limits are stated here without a priority claim. The certificate is not a Lean proof.
