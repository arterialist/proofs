...
This written counterexample concerns the [mixed additive completion](actual-source-mixed-dirichlet-completion.md) and the [centered prime multiplier](centered-prime-mellin-domain.md). It uses the classical PNT and an explicit auxiliary profile. Its conclusion does not concern the sign or convergence of the actual arithmetic source. It is not a Lean formalization.
