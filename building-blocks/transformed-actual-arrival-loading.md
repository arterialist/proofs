...
This written proof combines the [large-birth energy theorem](infinite-centered-birth-energy.md), the [original centered multiplier domain](centered-prime-mellin-domain.md), and classical PNT with exact finite arithmetic increments. It separates the actual prime, proper-power and density-cell contributions and keeps the ramp cross terms. The cumulative loading is proved; the signed past-arrival cancellation required for bounded transformed energy is not. No Lean formalization of this analytic theorem is claimed.
...
\tag{H}
$$
Theorem (H) is proved in [infinite centered birth energy](infinite-centered-birth-energy.md), equation (3), in the same original additive-coordinate energy used here. The loading conclusions below follow from that theorem, the established original Mellin-domain construction in [centered prime mellin domain](centered-prime-mellin-domain.md), and classical PNT. They are unconditional. The ramp and domain estimates in sections 1--2 do not use (H).
...
$\mathcal U^{-1}C_\infty\mathcal U$, with the original-space multiplier from [centered prime mellin domain](centered-prime-mellin-domain.md). In this note the same symbol $C_\infty$ denotes this conjugated realization. Its sufficient domain is controlled by
$$
|c_\infty(\nu)|\le C\Omega(\nu),\qquad
\Omega(\nu)=1+\log_+\frac1{|\nu|}+\log\log(3+|\nu|),
\qquad \nu\ne0.
