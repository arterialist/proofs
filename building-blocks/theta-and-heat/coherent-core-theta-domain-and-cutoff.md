...
$$
 ...
 \tag{1}
$$
Retain the [explicit two-source family](../weil-and-spectral/actual-core-explicit-weil-cross-family.md)
$$
...
The [scaling kernel](../successor-and-division/successor-feedback-kernel-scaling.md), [explicit signed factorization](../weil-and-spectral/actual-core-explicit-weil-cross-family.md) and [theta jump form](theta-weil-jump-form.md) fix the actual sources and normalization. The complete prime term equals twice the [coherent core observation](../prime-history/coherent-core-positive-observation-inner-history.md); its physical source cutoff is the [signed port](../prime-history/positive-history-observation-original-cutoff-port.md). The covariance and polarization identities, bounded-variation Fourier estimate and weighted Cauchy–Schwarz factorization bound are classical tools.
...
The [exact radical projection](../weil-and-spectral/actual-core-Weil-arch-row-radical-projection.md) removes both source pole moments while preserving the full physical Weil pairing. Its prime-port constant is exactly compensated by the archimedean row. This supplies a pole-null pair but does not remove the theta-domain obstruction or prove its cross-row sign.
