...
\]
where \(K\) is the [actual successor scaling kernel](../successor-and-division/successor-feedback-kernel-scaling.md). The full prime-power sum, both pole products, the archimedean multiplier and the hard core are retained.
...
\[
 ...
 \quad}                                                        \tag{1}
\]
This improves the unconditional [Goldbach-norm mixed-row estimate](../goldbach/goldbach-scaling-kernel-hard-wall.md) \(O_{c'}(t^{1/2-c'})\), \(c'>1\), for this specified test. It supplies no sign or RH conclusion.
