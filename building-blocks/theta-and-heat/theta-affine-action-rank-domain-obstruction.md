...
There is a source-domain improvement. The [exponential-growth full-rate lemma](theta-suzuki-variation-full-clock-consumer.md#exponentially-growing-smooth-odd-potentials-are-operator-tests) proves \(S_y,S_q\in D(K)\), and the separate [crossing-source action proof](theta-score-crossing-second-clock-domain.md) proves \(S_f\in D(K)\). Since \(K_0^{-1}\) is a scalar multiple of the identity plus a rank on the sign vector \(\varepsilon\in D(K)\), the bare Woodbury source \(z_s=K_0^{-1}(\delta S_y-t_{\rm score}S_q)\) lies in \(D(K)\). The complete prime profile and matched correction columns still enter \(v_U\), so these upgrades do not imply \(v_U\in D(K)\) or bound \(n[v_U]\).
...
\tag{17}
\]
The separate [crossing-source action proof](theta-score-crossing-second-clock-domain.md) establishes the left side of (17), hence \(\varphi\in D(K^2)\). Combining (16)--(17) yields a sufficient regularity condition for the entire bounded-model optimizer:
\[
