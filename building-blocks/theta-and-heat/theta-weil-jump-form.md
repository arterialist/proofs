...
[PhysicalThetaSeed.lean](../../formalization/BuildingBlocks/PhysicalThetaSeed.lean) additionally proves the first and second derivatives of each literal Gaussian seed a_n(u)=exp(u/2)exp(-pi n² exp(2u)), and a_n''(u)-a_n(u)/4=Phi_n(u), including the zero-index cancellation. It proves seed and curvature summability and the resulting scalar sum identity. See [the precise differential foundation](physical-theta-seed-differential-foundation.md). Infinite derivative interchange, Poisson modular identification and parity remain separate unfinished steps.

[PhysicalThetaUniform.lean](../../formalization/BuildingBlocks/PhysicalThetaUniform.lean) now proves both differentiations through the actual Gaussian seed series from summable fixed-interval derivative envelopes. Its genuine seed-sum differential identity recovers the literal Phi, and the same local envelopes prove full real-line continuity of Phi. See [the actual derivative-interchange scope](physical-theta-local-derivative-interchange.md). Parity, higher regularity, xi transform, moment integrability and the actual closed jump operator remain unfinished formalization steps.

[PhysicalThetaParity.lean](../../formalization/BuildingBlocks/PhysicalThetaParity.lean) proves true evenness and full-line positivity of the literal series via the real Gaussian Poisson theorem, with both differentiated identities and the zero integer term retained. See [the proof and scope](physical-theta-true-poisson-parity.md). This closes the parity and positivity steps listed above; higher regularity, exponential moments, xi identification, the closed jump operator and its sharp gap remain unfinished.

[Actual theta exponential moments](physical-theta-exponential-moments.md), compiled in [PhysicalThetaMoments.lean](../../formalization/BuildingBlocks/PhysicalThetaMoments.lean), proves integrability of exp(beta |u|) Phi(u) for every fixed real beta from a literal Gaussian bound. It also proves bounded measurable multiplier and exterior-indicator consumers. This closes the moment-integrability prerequisite listed above; actual killed-ground construction, boundedness, the full ground equation, xi identification and arithmetic residual smallness remain unfinished.

[The equality-tower finite-evaluation argument](theta-finite-vertex-factorization-and-return-obstruction.md) supplies a parity-specific alternate proof of the finite-vertex square obstruction in section 10. It also proves that the actual semigroup cannot satisfy positive-kernel domination P_t >= (1-exp(-t/2))Π at any finite t>0: the residual would fix both separating equality towers, force an identity kernel, and contradict local logarithmic smoothing. This is reviewed written mathematics awaiting Lean. It excludes that exact return comparison, without excluding sharp spectral decay, smaller or limiting comparisons, nonlocal factorizations or the unresolved theta gap.
