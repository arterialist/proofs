...
RH Proof supplied these proofs using standard finite sums, real exponential estimates, Cauchy-envelope domination and positive-axis change of variables, with no priority claim. The budget K_N establishes finite-horizon integrability and supplies no RH-scale growth estimate. The written factorial-kernel theorem identifies E_N=Q_N; the log-binomial integral component of that dictionary is now compiled in [FactorialDensityIntegral](BuildingBlocks/FactorialDensityIntegral.lean), while the entropy component and complete finite quadratic dictionary remain separate Lean obligations. The arithmetic bound on W_(N,b)-R_N and the actual RH implication remain unproved or unformalized as indicated in the companion chapter. This formal infinite identity does not establish the missing signed upper bound.
