...
Let $\lambda=\int F>0$. The [complete-process variation estimate](dyadic-prime-process-variation.md) and the [exact correlation formula](actual-short-time-psi-correlation.md) give
...
When the time exponent converges, (16) proves (2). The separate [Mellin source calibration](actual-short-time-psi-correlation.md) identifies $\kappa_0=2\Theta-1$, where $\Theta$ is the supremum of the real parts of the nontrivial zeta zeros. Thus
...
The [complete birth Gram](original-l2-evolved-source-gram-work.md) retains the corresponding signed loading and terminal work. The [literal successor renewal](literal-compensated-successor-iterates.md) is a different finite-source construction; neither identity supplies the unevaluated subpower estimate.
