...
The all-prime original-$L^2$ convergence follows from (8)–(10), finite source-measure variation, and the literal $B\in L^2$ theorem in [the seed construction](positive-successor-seed-jump-process.md). Its formal counterpart is [allPrimeSeed_memLp_two](BuildingBlocks/PrimeSeedAllPrimeSquare.lean). In negative age the shifts of $B$ have an $L^2$ convolution majorant, so prime-cutoff dominated convergence applies. On $x>1$, the displayed $x^{-2}$ bound gives the same conclusion. Physical $L^1$ on $(0,1)$ follows by Cauchy–Schwarz, and on $(1,\infty)$ by the $x^{-2}$ bound. Thus the full original-energy expression and every retained kernel integral are finite.
