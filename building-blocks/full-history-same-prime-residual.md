...
The eight adapted modules in this dependency chain compile in the central repository. The integral probability theorem is about the literal complete prime-power sum (17d), with all inactive/birth terms proved zero. Its full identification with a filtered prime-block convolution is a separate formal integration step. The written equalities (18)–(21), PNT asymptotics, polynomial estimate, and RH consumer remain outside this Lean batch. The methods use classical finite-sum inequalities, differentiation on exact cells and integration of monotone functions; no priority claim is made.
