...
This written proof applies the [large-birth energy theorem](infinite-centered-birth-energy.md) and the [actual loading calculation](transformed-actual-arrival-loading.md) to any fixed probability law of nonnegative ages. It includes the [complete positive successor process](positive-successor-seed-jump-process.md), allowing infinite mean age. It preserves the leading loading rather than proving bounded transformed source energy. These are analytic proofs, not Lean theorems.
...
\tag{1}
$$
This includes every fixed time of the all-prime positive age process in the [positive successor-seed process](positive-successor-seed-jump-process.md). All its compound-Poisson histories are retained in $\eta$. The measure is held fixed in every limit below.

The [proved dilation bound](positive-successor-seed-jump-process.md) $\sup_{0<a\le1}\|D_a\|_{\mathcal E\to\mathcal E}\le C_{\mathcal E}$ gives $\|\mathcal T_\eta\|_{\mathcal E\to\mathcal E}\le C_{\mathcal E}$. On original $L^2(dx)$, it is a contraction by the unitary dilation norm and the probability normalization. No first age moment is required.
