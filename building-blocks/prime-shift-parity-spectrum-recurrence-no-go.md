...
There is a direct obstruction for the [continuum-prime square](suzuki-odd-continuum-prime-square-and-arithmetic-remainder.md). Write \(P_a^{\rm odd}=P_{a,\mathrm{cont}}^{\rm odd}+\mathcal R_a^{\rm odd}\), with \(P_{a,\mathrm{cont}}^{\rm odd}=-\mathcal G_a\le0\) and \(\mathcal G_a\) a continuous-kernel compact operator. Weyl's compact-perturbation theorem and the parity result give the exact essential-spectrum identity
\[
