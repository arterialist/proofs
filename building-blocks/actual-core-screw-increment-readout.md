...
The source conventions are those of [the complete kernel](successor-feedback-kernel-scaling.md), [the correlated cross family](actual-core-explicit-weil-cross-family.md), [the full archimedean row and radical projection](actual-core-Weil-arch-row-radical-projection.md), and [the positive integer-birth quadrature](actual-integer-birth-positive-quadrature.md). Every von Mangoldt sum below includes all prime powers.
...
The [Matsumoto–Suzuki source map](goldbach-literal-successor-counting-transport.md#primary-literature-connection-and-its-limits), [complete scaling kernel](successor-feedback-kernel-scaling.md), [correlated cutoff](coherent-core-theta-domain-and-cutoff.md) and [radical projection](actual-core-Weil-arch-row-radical-projection.md) fix the original row and all contacts. The [positive birth quadrature](actual-integer-birth-positive-quadrature.md) retains its correction in the scalar readout. This is a written identity and a general screw-cone functional counterexample, not a counterexample to the actual zeta screw property. The [factorial-ground configuration decomposition](factorial-ground-screw-increment-consumer.md) gives another positive square with its separate full residual; neither construction signs the remaining actual observation. No Lean formalization of these screw readouts is asserted.
