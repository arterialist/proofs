"""Arb certificate: odd positivity through a=log(3)/2.
...
for i in range(N):
    for j in range(N):
        R[i, j] = (arb(fmpq(997, 1000)) if i == j else arb(0)) - Q[i, j]
_, pivot_r = cholesky_verified(R)

remainder = Fraction(66, 5) ** 49 / factorial(49)
assert 5 * remainder < Fraction(1, 1000000)
assert (arb(fmpq(997, 1000)).sqrt() + arb(fmpq(1, 1000000))) ** 2 < arb(fmpq(499, 500))
...
print("smallest 0.997I-Q pivot:", pivot_r)
print("Taylor remainder:", remainder)
print("CERTIFIED: L_a^odd >= 1/625 I for 0 < a <= (log 3)/2")
