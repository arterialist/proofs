...
The last term vanishes for $h\le X$. The two signed endpoint interactions do not vanish. The added endpoint masses are error values, so the $\log W$ exception estimate for positive prime-power weights cannot be applied to them. The boundary-square terms and the large-scale squared mean from the [complete Haar decomposition](../prime-distribution/haar-prime-energy-completion.md) also remain. No full mean-square improvement is inferred from S22.
