...
The [literal causal successor and charge](../successor-and-division/actual-successor-resonance-charge-cocycle.md) give $Sg(x)=g(x+1)$ for $x\ge1$ and zero below one. Its compensated version is $Cg=Sg-\kappa/x$ on $x\ge1$, where
...
Here $F$ is the [actual factorial potential](../factorial-and-renewal/factorial-ground-state-original-l2-domain.md). Write $S(v)=H_1(e^v)$. Differentiating (11) using the literal $F′$ gives, for $v>0$,
...
[`GoldbachSuccessorClampFinite.lean`](../../formalization/BuildingBlocks/GoldbachSuccessorClampFinite.lean)
