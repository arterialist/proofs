...
\tag{1}
\]
This is the exact [prime cross profile](../theta-and-heat/theta-cross-arrival-density-calibration.md) with both physical endpoints restricted to the collar. The full ground clock has \(d\gamma(u)=e^{-u/2}g_R(u)du\) on the positive half. Write \(d\gamma_c=e^{-u/2}g_c(u)du\) there and reflect it evenly.
...
Let \(c_{\rm dens}=\mu_R-a_R=\int e^{|u|}d\gamma\), the density coefficient in the [actual profile calibration](../theta-and-heat/theta-cross-arrival-density-calibration.md). For the compact row set
