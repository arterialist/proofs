...
The [complete early-divisor sieve](../prime-distribution/large-prime-additive-sieve-and-indefinite-kernel.md) gives `L_X(n)=(log n)Σ_{d|n,P⁺(d)≤z}μ(d)`. Thus `L_X` consists of the fresh prime arrivals and `Q_X` consists exactly of primes at most `√X` and every higher prime power. For ordered pairs write
