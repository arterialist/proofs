...
$$
 ...
 \alpha=M-L,\qquad T_*=\tfrac13\log150.
$$

**Theorem.** For every
$$
 \frac12\log2<a<\frac16\log150,
\tag{1}
$$
...
If $\log5\le T<T_*$, put $e=T-\log5$ and $\beta=\log5-2L=\log(5/4)$. The $n=5$ edge begins on the coordinate arc $A_5=(0,e)$ and ends on $A_5+\beta\pmod L$. Let
$$
 U=b(A_5)\cup b(A_5+\beta)\subset B.
\tag{7}
$$
Because $b$ is piecewise a translation, its image does not increase arc length, so $|U|\le2e$. But
$$
 |B|=L-d=\log6-T>2(T-\log5)=2e
 \quad\Longleftrightarrow\quad T<T_*.
\tag{8}
$$
Thus $B\setminus U$ contains a nonempty interval. The parity reflection reverses every $n=5$ edge: $Q(\theta+\beta)=e-\theta\pmod L\in A_5$ for $\theta\in A_5$. Hence the induced $q$ preserves $U$ and its complement. Choose a small interval $J\subset B\setminus U$ on which $q$ is a reflected translation, $q(J)\cap J=\varnothing$, and both $J,q(J)$ avoid the finitely many breakpoints. This choice makes every $n=5$ graph difference zero because both endpoint labels lie in $U$, where the functions below vanish.
...
The result places a concrete obstruction throughout $\tfrac12\log2<a<\tfrac16\log150$. The remaining interval up to $\tfrac12\log6$ is not settled here. The construction is a graph/domain statement, not a negative full-Weil witness or an RH argument. No priority claim is made, and no Lean formalization is claimed.
