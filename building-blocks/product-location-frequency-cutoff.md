...
Use $a(s)=e^{-s/2}(\psi(e^s)-e^s)$, with $a(0)=-1$, and the fixed causal cutoff $f_t=\chi_ta$ from the [successor theorem](actual-successor-frequency-cutoff.md). Here $t\ge1$, $\chi_t=1$ on $[0,t]$, decreases linearly to zero on $[t,t+1]$, and is zero thereafter. The exact derivative measure is
