...
To reproduce the certificate, install Python 3 and python-flint 0.9.0, then run, without Python's optimization flag:
    ...
    python building-blocks/certificates/theta_mellin_parabola_violation.py
...
It does not exclude a [grouped-factor construction](grouped-quadratic-multipliers.md): real factors can compensate a quadratic that fails the individual test. Neither such a factorization for the actual function nor RH follows from the present result.
