...
\tag{2}
```
The bound in (2) is proved in [HyperbolaMertens.lean](BuildingBlocks/HyperbolaMertens.lean), theorem `centeredDivisorForcing_sqrt_bound_all`, using the explicit hyperbola estimate in [DivisorForcingBound.lean](BuildingBlocks/DivisorForcingBound.lean). For the actual weights $\mu$, the finite Möbius inverse identity gives
```math
