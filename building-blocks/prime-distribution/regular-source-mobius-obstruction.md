...
The extra regularity is a uniform bound on first differences; it does not assert differentiability of a real extension. The theorem is formalized in [RegularSource.lean](../../formalization/BuildingBlocks/RegularSource.lean). The proof below preserves its finite construction and the distinct Banach–Steinhaus step that produces one fixed sequence.
...
Let $H(q)=\log(q!)-\sum_{n\le q}d(n)+2\gamma q$, where $d(n)$ counts positive divisors and $H(0)=0$. Its square-root bound is proved in [DivisorForcingBound.lean](../../formalization/BuildingBlocks/DivisorForcingBound.lean), and its exact inverse is the prime error. The connection to the particular arithmetic forcing is explicit. Define
...
The estimate needed for RH remains the signed bound for I_H(N)=ψ(N)−N+2γ, as also expressed by the [finite Mertens-to-prime transfer](../catalog.md#finite-mertens-to-prime-transfer). No bound for I_H at the RH scale, and no conclusion about zeta zeros, follows from this obstruction.
