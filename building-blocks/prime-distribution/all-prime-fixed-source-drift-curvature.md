...
This proves the infinite-prime passage left open in the [finite-prime potential construction](../dynamics-and-feedback/positive-primitive-two-port-drift-curvature.md), for each fixed natural source
...
B_P=\sum_{p\le P}B_p,\qquad B=\sum_pB_p,\qquad
 ...
 \tag{1}
$$
The [completed seed theorem](../successor-and-division/positive-successor-seed-jump-process.md) gives $B\ge0$, $B\in L^2$, and
$$
...
Put $d_N=b_N-a_N=P_N'$. The [natural-source primitive theorem](../successor-and-division/natural-cutoff-successor-primitive.md)
...
The [original metric calculation](../dynamics-and-feedback/positive-primitive-two-port-drift-curvature.md#3-exact-metric-derivatives-expose-the-ordinary-successor-again) gives
$$
...
midpoint pairings remains unproved. The separate [uniform correction theorem](../successor-and-division/full-prime-successor-potential-midpoint.md) proves convergence of the successor increment; it does not assert convergence of the individual midpoint columns. The reflected W readout and its same-prime subtraction
...
For measurable real $H$, with extended nonnegative energy allowed, [MixedEnergySuccessorStep](../../formalization/BuildingBlocks/MixedEnergySuccessorStep.lean) proves `unit_step_le_shortEnergy` and `unit_step_le_energy`:
...
[MixedEnergyBilinear](../../formalization/BuildingBlocks/MixedEnergyBilinear.lean) defines the measure
...
[SuccessorCriticalLift](../../formalization/BuildingBlocks/SuccessorCriticalLift.lean), in namespace `BuildingBlocks.SuccessorCharge`, proves the literal coordinate factors for $\sigma(v)=\log(1+e^v)$. Its `successor_predecessor_adjoint` retains the causal predecessor's zero initial interval $u\le\log2$. The two compositions are exactly $SP=I$ on positive ages and $PS=\mathbf1_{(\log2,\infty)}$. The square-integral identities allow infinite nonnegative totals; the two `MemLp` preservation theorems have measurable half-line $L^2$ hypotheses.

[CriticalCoordinatePairing](../../formalization/BuildingBlocks/CriticalCoordinatePairing.lean) proves the pairing and square change of variables from $v>a$ to $x>e^a$. Its successor pairing uses $x>1$ and its predecessor pairing uses $x>2$. The theorem `critical_contact_identity` identifies the physical contact form with the two logarithmic port pairings when the two physical inputs are $L^2$ and separately vanish almost everywhere below $1$. `critical_contact_bound` then gives the constant $9$ for measurable such inputs.
