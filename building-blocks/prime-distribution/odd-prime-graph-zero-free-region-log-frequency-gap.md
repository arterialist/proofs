...
The scale limitation survives the [sparse coherent-weight countermodel](sparse-prime-weight-ground-positive-terminal-sign-countermodel.md). In that model \(\sum_{n\le X}|\Lambda_*(n)-\Lambda(n)|=o(X)\) because the prime-base multipliers tend to one; partial summation gives \(\sum_{n<X}|\Lambda_*(n)-\Lambda(n)|/\sqrt n=o(\sqrt X)\). Thus its twisted prime multiplier differs from the actual one by \(o(e^a)\) **uniformly at every frequency**, so the leading pairing estimate above persists, while the same fixed pole-null terminal score has two-sided excursions exceeding \(e^{\theta a}\) for every \(\theta<1\). Any passage from this graph estimate to the required bounded signed score must use finer arithmetic information.
