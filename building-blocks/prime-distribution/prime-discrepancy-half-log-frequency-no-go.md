...
[the odd continuum note](../weil-and-spectral/suzuki-odd-continuum-prime-square-and-arithmetic-remainder.md):
...
This contradicts (2).
Quantitatively, if the coefficient of $\mathfrak H_{\alpha,a}$ in (2)
is allowed to depend on $a$ while $D=O(1)$, this carrier forces
$C_a=\Omega_\alpha(a^\alpha e^{(1-2\alpha)a})$. At the borderline
$\alpha=1/2$, it forces $C_a=\Omega(\sqrt a)$.
...
This is a written proof, not a Lean formalization. No novelty or priority is claimed pending comparison with existing prime-phase recurrence estimates.
