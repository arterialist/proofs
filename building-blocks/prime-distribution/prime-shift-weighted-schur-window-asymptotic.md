...
One could use \(C_a\) in a constant-baseline splitting of the archimedean multiplier, as in the [odd half-window certificate](../weil-and-spectral/suzuki-reflected-two-half-window-positive-certificate.md). Since that multiplier grows like \(\log(t/(2\pi))\), the corresponding safe cutoff still scales as \(\exp((1+o(1))e^a)\). Thus (3)–(6) improve the exponent constant in that particular reduction but retain a doubly exponential cost. They establish no positivity of the Weil form for a new window and imply nothing about RH.
