...
Unlike the [earlier one-sided estimate](../analytic-methods/signed-smooth-same-horizon-large-constant.md), this bound has no fixed leading-order localization loss. Its proof uses a complex stationary phase and a compact Fourier filter; a positive-probability tail bound alone is too weak at this scale.
...
The forward implication gives strict negativity. Neither (1) nor (2) proves that sign unconditionally. Equality $c=c_*$ is not covered by the positive leading coefficient. For $c<c_*$, the [squarefree support argument](../dynamics-and-feedback/log-squared-smooth-transport-boundary.md) rules out eventual nonpositivity.
...
The [rough future-cone covariance calculation](rough-future-cone-covariance-load.md) quantifies how much negative coupling the missing sign would require, including a superlinear positive block-energy load along square observation horizons.
