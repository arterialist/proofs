...
This identity is formalized in [PrimePrimitiveFormula](../../formalization/BuildingBlocks/PrimePrimitiveFormula.lean). In [PrimePrimitiveContraction](../../formalization/BuildingBlocks/PrimePrimitiveContraction.lean), integration by parts gives, for $T\ge1$,
...
Thus $Q_T\le E_T\le9Q_T$ for $T\ge0$. The proof expands the squares and uses $2\int ra=r(T)^2+3\int r^2$. See `primeLogFilter_energy_comparison` in [PrimeLogFilterReverse](../../formalization/BuildingBlocks/PrimeLogFilterReverse.lean), with the actual regularity in [PrimeLogFilter](../../formalization/BuildingBlocks/PrimeLogFilter.lean). The more general rate-$\lambda$ integration-by-parts identity is in [CausalFilterEnergy](../../formalization/BuildingBlocks/CausalFilterEnergy.lean). This is a standard first-order filter estimate, specialized without a smoothness assumption on the prime-error input.

The exact first-moment formulas, including terminal terms, are in [PrimeFirstMoment](../../formalization/BuildingBlocks/PrimeFirstMoment.lean):
...
[SelbergCenteredPairing](../../formalization/BuildingBlocks/SelbergCenteredPairing.lean) defines this exact finite expression; [SelbergReflection](../../formalization/BuildingBlocks/SelbergReflection.lean) proves, for integer $N\ge1$,
...
These are `selbergCenteredArea_sq_le_filtered` and `selbergCenteredArea_sq_le` in [SelbergReflectionBound](../../formalization/BuildingBlocks/SelbergReflectionBound.lean). The norm equivalence above does not reverse (3): a small reflected bilinear integral need not imply a small input norm. [SelbergLogReflection](../../formalization/BuildingBlocks/SelbergLogReflection.lean) records the logarithmic-coordinate version. [ReflectedConvolutionTransfer](../../formalization/BuildingBlocks/ReflectedConvolutionTransfer.lean) and [PrimeLogConvolutionTransfer](../../formalization/BuildingBlocks/PrimeLogConvolutionTransfer.lean) retain finite $L^2$ approximation errors. [SelbergSpectralModes](../../formalization/BuildingBlocks/SelbergSpectralModes.lean) evaluates each finite pair of exponential modes, including the resonant term $T e^{i\gamma T}$ when their frequencies coincide; no convergence of an infinite zero expansion is assumed.
