...
The coefficient tilt follows from the complete one-prime geometric series; the endpoint-dependent choices and their Chebyshev bounds are proved above. The regularized half-line formula and its finite-band interference terms are from [Complete prime histories as causal delay overflow](../prime-history/causal-history-overflow.md), equations (4)--(8); no new regularized trace formula is asserted here. The two-history sources and the warning about the complete boundary sum are from [Two-prime unilateral transport and the common resonance](two-prime-unilateral-boundary.md). The age substitution is from [An age-resolved lift of the actual prime-error history](../prime-history/age-resolved-prime-history-lift.md).
