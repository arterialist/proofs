...
The exact inverse is also used in the [finite Mertens transfer](../catalog.md#finite-mertens-to-prime-transfer). Here the divisor lengths may be much larger than $\sqrt N$.
...
This explains a limitation of estimating every divisor range separately. It does not require the terminal block to be estimated separately in a future proof: cancellations between the ranges remain possible. The [small-prime grouping theorem](small-prime-grouping.md) tests one such attempted regrouping, retaining its exact signed inner sums. The [hyperbola-product theorem](../successor-and-division/hyperbola-product.md) uses the different product variable $k=nm$ and controls products through $N^{5/8}$; it does not replace the divisor-block estimates here.
