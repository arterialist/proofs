...
The covariance and convergence arguments are written proofs. The individual-profile integrability and square-mass normalization (7) are formalized in [PrimeSeedSquare.lean](../../formalization/BuildingBlocks/PrimeSeedSquare.lean); the [exact theorem mapping](../successor-and-division/positive-successor-seed-jump-process.md#formalized-single-clock-dependencies) identifies the remaining written dependencies. This is an exact finite-prime identity and a fixed-observation convergence theorem. It uses the complete critical prime factors, including every proper power. The boundary profile is the actual initial-seed response established in [successor/source compatibility theorem](../successor-and-division/successor-collective-prime-compatibility.md); its longer memory is analyzed in [long-memory theorem](../successor-and-division/collective-successor-seed-long-memory.md). The Cauchy characteristic function, Fourier uniqueness, Plancherel, and the elementary one-pole causal all-pass isometry are classical tools. No novelty claim is made for those tools or their general abstract transform; the stated arithmetic specialization retains the actual complete prime coefficients and successor boundary seed.
