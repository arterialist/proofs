...
This proof converts a lower bound for the complete harmonic phase defect into a finite-prime divergence for the actual mean-zero line-one periodization. It uses the physical generator amplitudes $p^{-1/2}$ from the start. The analytic statements below are written proofs. The finite factorization, harmonic averaging and arithmetic coefficient bounds have the Lean formalizations specified at the end.
...
\tag{2}
$$
The input is equation (8) under “An exponential variation bound also suffices” in [the harmonic-convolution theorem](../analytic-methods/harmonic-convolution.md). In its notation, for fixed $C,K>0$ and $0<\varepsilon<1/2$,
$$
...
[PrimePhaseFactorization.lean](../../formalization/BuildingBlocks/PrimePhaseFactorization.lean) proves the following declarations in the namespace `ArithmeticDynamics`:
...
[HarmonicPrimePhase.lean](../../formalization/BuildingBlocks/HarmonicPrimePhase.lean) continues the same definitions and proves `harmonicPhaseDefect_le_prime_sum`, the exact normalized inequality (3), for every integer cutoff $N\ge1$ and real frequency. Its `valuation_overlap_eq_lcm_sum` counts $v_p(n)v_q(n)$ by common prime-power divisibility. The theorem `same_prime_valuation_sq_eq_max_sum` uses $p^{\max(i,j)}$, rather than $p^{i+j}$, with the explicit hypothesis $n\ne0$.

[HarmonicPrimeWeights.lean](../../formalization/BuildingBlocks/HarmonicPrimeWeights.lean) proves `harmonic_multiples_sum`, `harmonic_valuation_overlap`, and `harmonicPrimeWeight_eq_lcm_sum`. The last theorem is (4) at integer cutoff $N$, written with the finite exponent ranges $1\le i,j<N$. These ranges contain every nonzero contribution. It requires that $p$ is prime and includes $N=0,1$ using the total inverse convention $0^{-1}=0$. At $N=0$ the harmonic mass and both sides are zero. The modules also prove the nonnegative weights, positive harmonic mass for $N\ge1$, and the common-cutoff support identities used by these theorems.

[HarmonicPrimeWeightBound.lean](../../formalization/BuildingBlocks/HarmonicPrimeWeightBound.lean) proves `harmonicPrimeWeight_le_closed`, exactly (5) for every integer $N\ge1$ and prime $p$, including primes larger than $N$. Its proof first retains the finite reciprocal-lcm sum, then separates distinct primes from the same-prime maximum. The exact shell identity `sum_pow_max_Ico` counts $2j-1$ ordered pairs with maximum $j$; the two nonnegative geometric sums give the displayed closed bound. No estimate for the number or reciprocal sum of primes is a premise.

Thus the finite factorization, harmonic averaging, exact coefficient formula and closed bound (5) are formalized. The harmonic frequency estimate, the upper estimate $b_p\le D(P)/p$, periodization, infinite-flow construction and its absolute-flux domain remain written mathematics. The factorization and minimum-energy arguments use the classical triangle and Cauchy inequalities; the analytic inputs are the linked harmonic theorem, the classical quantitative prime number theorem and Leong's explicit logarithmic-derivative bound. No priority or first-formalization claim is made.
