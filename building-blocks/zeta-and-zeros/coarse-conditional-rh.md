...
Here $`\Lambda`$ is the actual von Mangoldt function, so proper prime powers are included. The definitions reuse [CoarsePrimitive.lean](../../formalization/BuildingBlocks/CoarsePrimitive.lean) without alteration. The dyadic index includes zero. Both primitives retain their endpoint means. No additional estimate at intermediate Haar scales is assumed.
...
Mathlib retains its upstream authorship and Apache 2.0 license. The extracted local proofs follow this repository's MIT license. Declaration-level extraction provenance and source hashes are recorded in [coarse-conditional-provenance.json](../../formalization/verification/coarse-conditional-provenance.json) and [coarse-conditional-sources.json](../../formalization/verification/coarse-conditional-sources.json).
...
~~~sh
python3 formalization/verification/verify-coarse-conditional.py --lake lake
~~~

The recorded result is in [coarse-conditional-result.json](../../formalization/verification/coarse-conditional-result.json). Verification uses the ordinary Lean kernel and pinned upstream compiled dependencies. It is not a fresh build of every mathlib dependency or a separate proof-checker implementation.
