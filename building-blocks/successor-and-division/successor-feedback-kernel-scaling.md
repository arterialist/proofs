...
The kernel $K$ is the same $k$ whose Mellin multiplier is evaluated in [the continuous-shift proof](../zeta-and-zeros/actual-Abel-feedback-Mellin-kernel.md). This chapter retains a distinct real-variable moment and one-crossing proof, and its stronger uniform coefficient error keeps the entire arithmetic endpoint correction when summed.
