...
The [dyadic extraction proof](../prime-distribution/dyadic-prime-process-variation.md) uses the full arithmetic tail to improve the complete-process rate to $t^{-1}$ and gives the finite-cutoff crossover. The fixed-component proof above remains a separate method requiring only one positive interval and uniform BV control.
