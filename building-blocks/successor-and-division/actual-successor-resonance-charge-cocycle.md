...
Split the integral at x=1/|xi| to obtain the last bound. Let $E$ be the [additive mixed Hilbert completion](../dynamics-and-feedback/actual-source-mixed-dirichlet-completion.md), with
...
Thus delta is a bounded functional on E, even though Q itself is not thereby bounded. The proved [natural-cutoff convergence](../dynamics-and-feedback/natural-arrival-cutoff-completion.md) $g_N\to g$ in $E$ proves that its canonical completed port is delta(g)=kappa, agreeing with the explicit arithmetic limit (4).
...
The function is continuous, constant d(1) below 1, and O(y^(-3/2)) at infinity. For a fixed prime $p$, put $L=\log p$. The complete local operator is the one in the [centered-kernel theorem](../prime-history/centered-infinite-prime-birth-kernel.md):
...
[SuccessorCharge.lean](../../formalization/BuildingBlocks/SuccessorCharge.lean) uses the literal physical-coordinate formulas
$$
...
[SuccessorChargeEnergy.lean](../../formalization/BuildingBlocks/SuccessorChargeEnergy.lean) proves the actual kernel estimate
$$
|k(u)|\le2e^{-u},\qquad
...
$$
Combining this with the [formal causal anchor](../dynamics-and-feedback/actual-source-mixed-dirichlet-completion.md#formalized-causal-energy-anchor), `defect_bound_causal_energy` proves, for measurable real $H$ vanishing almost everywhere below $1$ and satisfying $\mathcal E(H)<\infty$,
$$
