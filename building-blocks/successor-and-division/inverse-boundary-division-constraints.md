...
This is a written mathematical proof, not a Lean formalization. The [successor cutoff](actual-successor-frequency-cutoff.md) and [semilocal normalization](../weil-and-spectral/semilocal-weighted-scattering.md) fix the actual input and operator conventions.
...
These are finite sums for every fixed T. They are the actual inverse strip in [two history bilinear storage](../prime-history/two-history-bilinear-storage.md), not an independently chosen boundary seed.
...
Let $v=e^{-s/2}b_2(s)=(2\beta(s+L)-\beta(s))/\sqrt2$ and $V=Hv$. The [formal actual integer divisor theorem](../factorial-and-renewal/division-renewal-dyadic-average.md) gives $V(\log N)<-\delta/\sqrt2$ for every integer N>=1, where $\delta=3/2-\log4>0$. In particular
...
Use the same finite input $f_T=\chi_Ta$ and the two opposite transports from [two history bilinear storage](../prime-history/two-history-bilinear-storage.md). Write $p_T=U f_T$, $q_T=U^{-1}f_T$, and, on the positive half-line,
