...
$$
The [natural-cutoff theorem](../dynamics-and-feedback/natural-arrival-cutoff-completion.md) gives
...
[SuccessorMemoryKernel](../../formalization/BuildingBlocks/SuccessorMemoryKernel.lean) defines the literal causal memory
...
[PrimeSeedMemory](../../formalization/BuildingBlocks/PrimeSeedMemory.lean) proves `finitePrimeSeed_memLp_two` from the existing exact integral $\int B_p^2=p^{-2}$. The theorem `finitePrimeSeed_memory_commutator` therefore applies to the actual $F=\sum_{p\in S}B_p$ for every finite set of primes, retaining all powers of each prime. No analytic assumption on this finite seed is left as a premise. There is no uniform-in-$S$ norm estimate in this module.

[SuccessorCompensatedL2](../../formalization/BuildingBlocks/SuccessorCompensatedL2.lean) proves $a_0,k\in L^2(0,\infty)$, absolute existence of $\delta(f)$ for $L^2$ inputs, and preservation of measurable half-line $L^2$ by $C$. Its `compensated_adjoint` identifies the literal adjoint
...
[CompensatedMemoryKernel](../../formalization/BuildingBlocks/CompensatedMemoryKernel.lean) proves
...
[MemoryChargeFubini](../../formalization/BuildingBlocks/MemoryChargeFubini.lean) proves absolute product integrability before interchanging the actual charge and memory integrals. For measurable real $F\in L^2(\mathbb R)$ and $f\in L^2(0,\infty)$, `defect_causalMemory` gives
...
[PrimeSeedSquareConvergence](../../formalization/BuildingBlocks/PrimeSeedSquareConvergence.lean) uses the strict cutoff $B_{<P}=\sum_{p<P}B_p$. At each age this equals $B$ for every sufficiently large cutoff. Positivity gives $(B-B_{<P})^2\le B^2$, so dominated convergence proves `cutoff_seed_square_error_tendsto`:
...
[PrimeMemoryChargeDecay](../../formalization/BuildingBlocks/PrimeMemoryChargeDecay.lean) proves that the actual seed vanishes below $\log2$, where the relative-charge kernel can be negative. Since $k(u+w)\ge0$ for $u\ge0$ and $w\ge\log2$, the transposed charge is nonnegative. Its exponential estimate is
...
[MemoryChargeEnergy](../../formalization/BuildingBlocks/MemoryChargeEnergy.lean) proves directly that the product $d_Bf$ is integrable whenever $f$ is measurable and $e^{-u/2}f(u)\in L^2(0,\infty)$, with
...
For measurable real $H$ vanishing almost everywhere below $1$, put $f(u)=e^{u/2}H(e^u)$. If $\mathcal E(H)<\infty$, the formal [causal energy anchor](../dynamics-and-feedback/actual-source-mixed-dirichlet-completion.md#formalized-causal-energy-anchor) supplies the weighted integral. Thus `allPrimeSeed_memory_charge_bound_energy` and `allPrimeSeed_memory_charge_rank_bound` give
...
[MemoryChargeCutoff](../../formalization/BuildingBlocks/MemoryChargeCutoff.lean) retains the literal remainder $R_P=B-B_{<P}\ge0$, with $B_{<P}=\sum_{p<P}B_p$. It vanishes for $v<\log P$. For natural $P\ge8$, the earlier Chebyshev bound $vB(v)\le C_0:=64\log2+30$ therefore gives
