...
In particular $M_\chi\Delta(u)$ is trace class if and only if (1) is finite for $v=\chi u$. The constants are uniform over the unitary multiplier. This is an $L^1$ second-increment criterion, distinct from the [$L^2$ first-increment energy](../weil-and-spectral/successor-clock-scattering-energy.md) that controls Hilbert–Schmidt norm. This is a written proof using Peller's classical theorem and an explicit sampling argument, not a Lean formalization.
...
This is the fixed rationally shifted normalization of [the signed collective phase](../weil-and-spectral/signed-scattering-tail-energy.md). Every power of each admitted prime is present, including $p^k>X$.
...
For a fixed everywhere-positive Schwartz observation, the [single-observation theorem](../weil-and-spectral/single-observation-scattering-criterion.md) therefore makes a subpower bound for (11) sufficient for RH. Conversely, RH implies that bound through the [discrepancy-to-norm theorem](../weil-and-spectral/prime-discrepancy-scattering-norm.md) and (2). This reformulation does not establish the bound. The [explicit compact observation](../weil-and-spectral/compact-positive-scattering-observation.md) can be used throughout.
