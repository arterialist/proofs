...
The existing [radius-two rational-gap certificate](theta-radius-two-rational-gap-propagation.md) and its [exact rational replay](verification/theta_radius_two_J3_rational_upper_check.py) prove the lower gap endpoint from the finite theta-series estimate.  This is usable immediately wherever only an outward interval for \(\alpha_2\) is required.
