...
Finite grouping and Möbius inversion give $\sum_{q=1}^Nc_N(q)=M(N)$ and $\sum_{q=1}^Nq c_N(q)=1$ for $N\ge1$. The definitions are shared with [MertensArithmetic](../../formalization/BuildingBlocks/MertensArithmetic.lean); [QuotientMass](../../formalization/BuildingBlocks/QuotientMass.lean) proves the weighted identity used here.
...
On the interval $(1/N,2/N)$, the smaller image is still $-2$, so the integral is $b_N^2\int_{1/N}^{2/N}(Nx)^{-2}dx=b_N^2/(2N)$. Endpoint values are irrelevant to this integral. These are `correctedImage_second_window` and `corrected_second_window_distance_lower` in [SecondWindow](../../formalization/BuildingBlocks/SecondWindow.lean).
...
then gives $M(N)=o(\sqrt N)$. For a quantitative proof of (4), if the left side has absolute value at most $\delta\sqrt N$ beyond a fixed cutoff, strong induction and $\sqrt{\lfloor N/2\rfloor}\le3\sqrt N/4$ give $|a_N|\le4\delta\sqrt N+B$, where $B$ bounds the finite prefix. This is [HalvingRecovery](../../formalization/BuildingBlocks/HalvingRecovery.lean).
...
The finite-prefix proof has constant $1/(1-r)$. See [ContractionRecovery](../../formalization/BuildingBlocks/ContractionRecovery.lean), `mertens_ratio_tendsto_of_fixed_quotient`.
...
These statements are already formalized in [FractionalApproximation.lean](../../formalization/BuildingBlocks/FractionalApproximation.lean): `dualWindowEnergy_eq_weighted`, `dualWindowEnergy_lower`, `mertens_sq_le_dual_error`, `mertens_sq_normalized_tendsto_of_dual_error`, and `dual_error_not_tendsto_of_mertens_oscillation`. The implication is unconditional except where the named nonconvergence theorem explicitly assumes the oscillation premise. The same dual approximation is defined in [Báez-Duarte, equation (1.19)](https://arxiv.org/html/math/0011254); Proposition 4.6 proves its full $L^2$ nonconvergence by the paper's unitary-operator method. The mean-window argument above is a different proof of the necessary arithmetic obstruction.
