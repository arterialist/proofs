...
The application to the critical arithmetic sum $W$ is an alternative proof of a known conditional leading term. The [square-root sign criterion](../zeta-and-zeros/sqrt-multiple-sign-criterion.md) already proves that asymptotic with the stronger error $O(x^{3/2}\log x)$ and establishes its eventual-sign equivalence with RH. No priority claim is made. These arguments are written analysis, not Lean formalizations.
...
This recovers the leading term of the [stronger existing Mellin theorem](../zeta-and-zeros/sqrt-multiple-sign-criterion.md) by a different mechanism. The generalization concerns the inputs allowed by the convolution argument, not a new RH equivalence.
