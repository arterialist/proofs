...
The bounds in [the second-log proof](../prime-distribution/simultaneous-second-log-divisor-envelope.md) check out:
...
The only convention change is the possible prime atom at the closed lower endpoint. The split at `d<y` in [the ceiling proof](../successor-and-division/integer-moving-ceiling-discrepancy.md) gives
...
The split `d<y` versus `d≥y` in [the PNT proof](../prime-distribution/uniform-rough-prime-pnt-partial-summation.md) is correct. The small-`d` side pays only a harmonic sum, and the large-`d` side is geometric. Its aggregate sampled norm is
...
The variation budgets in [the sampling proof](../dynamics-and-feedback/sampled-green-norm-uniform-proof.md) are consistent:
