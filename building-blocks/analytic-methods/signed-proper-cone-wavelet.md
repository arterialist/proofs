...
Use the proper-cone kernel $L_{X,f}(d)=\sum_{2\le m\le X/d}(f(dm)-\bar f_X)$ from the [proper-cone note](../prime-distribution/proper-prime-cone-zero-preserving-slack.md). For $f(n)=\log n$ write $\ell_X=X^{-1}\log(X!)$ and
