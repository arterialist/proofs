...
The [incomplete carry-covariance formulas](../successor-and-division/incomplete-carry-covariance.md) retain finite averaging kernels and explicit endpoint terms instead of replacing them by complete-period values. The [actual temporal divisor-history covariance](../prime-distribution/temporal-floor-covariance.md) is a different kernel: its positive terminal-prime subblock is canceled to leading order by the full signed complement. Neither construction is replaced by the all-one sawtooth example.
