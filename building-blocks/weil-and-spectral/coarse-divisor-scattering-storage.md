...
The [successor-clock normalization](../successor-and-division/successor-counterphase-trace-norm.md) proves that, before centering, $\tau-\log$ is the divisor average of $1-\Lambda$, with its $n=1$ term included. Its Dirichlet series is obtained by multiplying $\zeta+\zeta'/\zeta$ by $\zeta$. Thus the coarse score has removed the logarithmic-derivative denominator. The stable coarse phase proves no bounded inverse for that operation, no estimate on the unaveraged prime score, and no positivity of the Weil form.
