...
We use [the actual cross-family construction](actual-core-explicit-weil-cross-family.md), the [complete kernel](../successor-and-division/successor-feedback-kernel-scaling.md), and the normalization of [the public theta Weil form](../theta-and-heat/theta-weil-jump-form.md). Here $t>0$ is the geometric successor parameter, and $\beta\in(1/2,1)$ is fixed.
...
\tag{14}
$$
This agrees with the exact constant in [the feedback calibration](../successor-and-division/successor-feedback-RH-converse.md).
...
This argument does not claim $u/\Phi$ or $z_t/\Phi$ belongs to the theta-weighted Hilbert space. The [independent cutoff calculation](../theta-and-heat/coherent-core-theta-domain-and-cutoff.md) retains the divergent theta cross norm and its cancelling jump term; none of those separate quantities is used here as a finite norm.
...
The [explicit two-source factorization](actual-core-explicit-weil-cross-family.md), [correlated theta cutoff](../theta-and-heat/coherent-core-theta-domain-and-cutoff.md), [complete scaling kernel](../successor-and-division/successor-feedback-kernel-scaling.md) and [actual theta Weil normalization](../theta-and-heat/theta-weil-jump-form.md) fix every term. The digamma integral and recurrence are classical; the displayed calculation keeps their complete prime, gamma and pole compensation. The extension of the radical to this exponential/BV source pair and its constant-scale correction are written proofs. No Lean formalization, finite theta norm for the uncut pair, or arithmetic gain from the projection is asserted.
