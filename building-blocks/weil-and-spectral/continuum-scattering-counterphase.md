...
Use the Hardy projection and trace convention of [semilocal weighted scattering](semilocal-weighted-scattering.md). Write $U_X=M_{u_X}$ for the actual archimedean factor times all local factors at primes $p\le X$, including every power in their logarithmic derivatives. Set $\Delta(U)=P-U^*PU$. The physical test stays fixed throughout this construction. This is a written proof. The theta kernel $\Phi$ has the normalization in the [derivative-kernel note](../theta-and-heat/theta-derivative-kernel-logconcavity.md); the theta image used below has norm $\|h\|_H^2=\int 2\cosh(u/2)\Phi(u)|h(u)|^2du$.
