...
\tag{4}
\]
The lower test is even; the [finite-shift parity-spectrum theorem](../prime-distribution/prime-shift-parity-spectrum-recurrence-no-go.md) transfers the same spectral edge to the odd restriction, including the pole-null class after a negligible high-frequency correction. Likewise $\|P_{k,a}\|=(1+o(1))a e^a$, using $k(N)=a+c+o(1)$, the weighted Schur bound for $P_a$, and the preceding positive even test followed by parity transfer.
