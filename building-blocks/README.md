...
[Compact-window certificate scope](compact-window-weil-certificate-carrier-scope.md) checks Zhu’s September 2026 v2 theorem statements against the actual q-flat carrier. Its nonzero endpoint packets are separated by (J+1)log2, so no translation fits the reported support-1.6 certificate for J>=3. This is a reviewed support argument and literature scope audit, not a recomputed certificate, Lean theorem or RH bound. Mixed packet terms remain uncontrolled by that small-window theorem alone.
