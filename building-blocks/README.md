...
[Logarithmic-window source pairing](q-flat-all-place-signed-compensation.md#formalized-logarithmic-window-calculus-and-original-source-pairing) adds sixteen compiled calculus and original frozen-source theorems with explicit support, derivative and cutoff hypotheses, including the exponential Jacobian and full compact readout. The signed estimate remains open. Separately, the [actual factorial successor counterexample](factorial-cross-size-mobius-energy.md#what-multiplicative-regrouping-does-and-does-not-cancel) proves by exact integers that Q_N-D_N increases from 4 to 5, ruling out monotonic compensation while leaving the integrated diagonal hypothesis open.

[Repeated raw-successor Goldbach leakage](goldbach-literal-successor-counting-transport.md#uniform-actual-pair-leakage-for-repeated-raw-succession) proves the uniform actual bound 0<=G(X+2j)-G_j(X)<=5C_psi^2j(X+2j), retaining early births at the causal boundary and the full continuous pair tails. Its cost becomes quadratic when j is comparable to X. It proves no RH-scale estimate and does not replace the signed ports of compensated iterates. This is written mathematics, not Lean-formalized.
