...
[Binary-horizon factorial signed work](factorial-mobius-quotient-renewal.md#binary-horizon-signed-work-with-exact-density-dissipation) retains every quotient-parity carry and proves Q_(2N+b)-2Q_N=W_(N,b)-R_N, with the exact positive density dissipation R_N. Actual Q_4>2Q_2 refutes the zero-work budget by integer comparison. A uniform linear signed-work budget would imply RH through all binary ancestries, but remains unproved. These are written results, not Lean-formalized.
