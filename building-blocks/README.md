...
[Repeated raw-successor Goldbach leakage](goldbach-literal-successor-counting-transport.md#uniform-actual-pair-leakage-for-repeated-raw-succession) proves the uniform actual bound 0<=G(X+2j)-G_j(X)<=5C_psi^2j(X+2j), retaining early births at the causal boundary and the full continuous pair tails. PNT proves the intrinsic leakage 2jX+2j^2+o(X^2) at j=floor(theta X), theta>0. The centered RH-scale remainder and signed ports of compensated iterates remain uncontrolled. This is written mathematics, not Lean-formalized.
