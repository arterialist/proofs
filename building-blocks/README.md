...
[Norm-preserving full Weil comparison at a concrete prime](norm-preserving-full-weil-prime-counterexample.md) refutes universal normalized finite-history monotonicity at p=10007 using actual prime-log phases, exact pole-null packets, gamma cancellation and a complete bound on positive proper powers. Both forms remain positive. Five finite arithmetic facts are compiled in Lean; the analytic argument and power-enumeration completeness remain written mathematics. No RH estimate or fixed-carrier failure is asserted.
