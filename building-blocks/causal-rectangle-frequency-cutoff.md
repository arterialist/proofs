...
Use the actual $a(s)=e^{-s/2}(\psi(e^s)-e^s)$, causally extended, and fix $t\ge1$. This is a written alternative to the support split in the [successor theorem](actual-successor-frequency-cutoff.md). Put $L=1+t$, $J=\lceil t\rceil$, and $R=\lfloor t\rfloor+2$. Let $\theta$ be the $C^1$ monotone step equal to zero below zero, equal to $3r^2-2r^3$ for $0\le r\le1$, and equal to one above one. Thus $0\le\theta\le1$ and $|\theta'|\le3/2$.
...
The integer-location translation argument in [the successor theorem](actual-successor-frequency-cutoff.md) applies to each $f_j$, including its clustered jumps. Hence, for $\Omega\ge2$,
$$
