...
The finite-band theorem supplies |∫_(0,T] interiorRow_N|≤NT/4 and hence ∫_(0,T] boundaryRow_N≤∫_(0,T] energyRow_N+NT/4 for every T≥0. Combined with the independently reviewed [intermediate-clock response estimate](factorial-intermediate-clock-signed-work-upper.md), this bounds the actual signed escaping boundary through T=N^(−1/2) by O(N log²N). The compensated logarithmic response estimate now compiles; the displayed combined logarithmic integral bound remains written. The stronger log-free Abel energy and boundary upper bounds are compiled in the linked refinement. The larger clock remains open.

[Reciprocal-prefix Abel cancellation](factorial-reciprocal-prefix-abel-clock-bound.md) now gives compiled Q_N(0,1/sqrt(N))≤9N/4 and signed B_N(0,1/sqrt(N))≤9N/4+sqrt(N)/4, with an actual fixed-constant quadratic common-dyad bound. These do not estimate the larger clock.
