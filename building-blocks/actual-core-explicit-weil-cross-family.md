...
Let $W(h)$ denote the full Weil functional and $A_{\rm pole,arch}(h)$ its pole plus archimedean terms, so $W(h_t)=A_{\rm pole,arch}(h_t)+2Q(t)$. Their complete integral expression is retained in the [correlated-cutoff theorem](coherent-core-theta-domain-and-cutoff.md).

The [integer-birth interpolation identity](actual-integer-birth-positive-quadrature.md) then yields
