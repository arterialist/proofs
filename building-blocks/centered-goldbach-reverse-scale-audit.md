...
The additive convolution, reverse-time variation-of-constants formula and critical-energy comparison above are written proofs. The PNT input is Johnston and Yang's Theorem 1.4, not a new zero-free region. The existing finite endpoint algebra in [GoldbachSuccessorTransfer.lean](BuildingBlocks/GoldbachSuccessorTransfer.lean) does not formalize the signed convolution, Duhamel formula or the analytic growth estimate here.
