...
All results here are written proofs, using classical Farey-neighbor, Möbius-product and elementary divisor identities. The exact common identities are also covered by the existing [boundary checker](verification/check-angular-boundary.py). No Lean formalization or priority claim is made for these additional estimates.
