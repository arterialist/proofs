...
The complete compensated weight bound w(t)≤1/(2t) is already compiled in [FactorialOriginNeutralization.lean](BuildingBlocks/FactorialOriginNeutralization.lean), and the unit-response energy log 2 follows from the compiled [full kernel dictionary](BuildingBlocks/FactorialKernelDictionary.lean). The actual integer-birth formula, decaying collision integral and bound, divisor-pair parametrization, linear total-birth bound, signed-work telescope and its RH consumer above are written proofs requiring further Lean formalization. No compiled theorem asserting the linear birth budget is claimed here. The full signed cumulative work estimate remains unproved; a linear quadratic-variation budget alone does not imply the RH energy bound.
