...
This written proof combines the [birth kernel](centered-infinite-prime-birth-kernel.md), [original multiplier domain](centered-prime-mellin-domain.md), and [fixed-Cauchy ground graph](fixed-cauchy-prime-covariance-seed.md). The arithmetic source cutoff is finite; its two temporal outputs retain their cross term. The [sharp cutoff law](fixed-source-centered-cutoff-law.md) gives the exact leading error for each fixed source. These analytic results are not formalized in Lean.
...
$$
where the actual kernels $A_P,A$ are constructed in [centered infinite prime birth kernel](centered-infinite-prime-birth-kernel.md). That construction proves
$$
