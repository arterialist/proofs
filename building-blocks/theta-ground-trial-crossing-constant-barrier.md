...
The way forward must change at least one ingredient: use the ground equation to extract an effective width strictly smaller than its certified support collar, improve the endpoint crossing comparison beyond its present operator norm, or prove the prime sign channel by a mechanism that does not pass through this width criterion. The calculation does not assert that the actual ground has width at least `2R e^(−R)`; it says this particular proof route cannot certify a smaller one with its current constants.
