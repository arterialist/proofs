...
The [unconditional PNT envelope](../prime-distribution/actual-short-time-psi-correlation.md) is
$$
...
Let $T_s$ be the [actual causal compound-Poisson evolution](../successor-and-division/positive-successor-seed-jump-process.md), conjugated to the physical space by Ug(v)=e^(v/2)g(e^v). Its complete exponent is Phi, with every proper-power interval of its Levy measure. It is an original-L2 contraction and preserves support to the right:
$$
...
\tag{12}
$$
Here is a direct Fourier proof retaining all frequencies. With rho(xi)=Re Phi(i xi), the complete process has rho(xi) comparable to |xi| near zero and a strictly positive lower bound outside a fixed small band. These are the [complete-kernel estimates](../dynamics-and-feedback/long-time-signed-source-rank-one.md) used in the [growing-time Gram proof](../dynamics-and-feedback/growing-time-actual-source-cocycle.md).
...
\tag{16}
$$
Their constants satisfy d_R+d_P-d_C=0 by (1). The [actual diagonal loading theorem](../dynamics-and-feedback/growing-time-actual-source-cocycle.md#complete-diagonal-loading)
$$
...
The bounded positive forms come from the [factorial domain and inverse](factorial-ground-state-original-l2-domain.md); the [Picone source proof](../dynamics-and-feedback/picone-frozen-source-work.md) supplies the initial source and every beyond-cutoff prime observation. The [complete process](../successor-and-division/positive-successor-seed-jump-process.md), [growing-time source cocycle](../dynamics-and-feedback/growing-time-actual-source-cocycle.md), and [clock-action criterion](total-clock-dissipation-and-signed-work.md) retain the original norm and clock normalization. PNT, Hardy convolution and Plancherel are classical inputs. All statements of this chapter are written proofs; no Lean coverage of these observation forms or loading asymptotics is asserted. The absolutely summable diagonal difference does not bound the mixed arithmetic work.

The [time-ordered trace-class extension](time-ordered-factorial-birth-trace-class.md) controls the entire polarized $P-C$ matrix, including the unequal admission clocks and the restored seed. Its coherent all-ones quadratic form is exactly the signed terminal-plus-clock budget. That operator-class constraint does not supply a subpower estimate for the growing coefficient vectors.
