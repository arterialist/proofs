...
[FactorialClockCofactorFinite.lean](../../formalization/BuildingBlocks/FactorialClockCofactorFinite.lean) compiles (3) from the exact displayed admissions and proves their one-step factorial/logarithm recurrence, including $m=0,1$. Its connection to the improper integral (1) still rests on the written evaluation (2).
...
$(m+N)\log(1+m/N)\to m$. Thus $k$ is an infinite-size boundary column of the compiled [factorial kernel dictionary](../../formalization/BuildingBlocks/FactorialKernelDictionary.lean), not a separate clock choice. The boundary column is not a finite-energy vector: Stirling gives
...
The last equality is the classical [partial fraction for $\coth$, DLMF 4.36.3](https://dlmf.nist.gov/4.36.E3). It proves $0<\omega_B(u)<1/12$ pointwise, with $\omega_B(u)\sim u^2/720$ at zero and $\omega_B(u)\to1/12$ at infinity. The strict positivity of the **explicit corrected original density**, independently of the partial-fraction theorem, is compiled in [FactorialBinetResidualDensityPositive.lean](../../formalization/BuildingBlocks/FactorialBinetResidualDensityPositive.lean): after multiplying by $12u^2(e^u-1)>0$, its numerator $H(u)=(u^2-6u+12)(e^u-1)-12u$ has $H(0)=H'(0)=H''(0)=0$ and $H'''(u)=u^2e^u>0$ for $u>0$. The exact Laplace identity is
