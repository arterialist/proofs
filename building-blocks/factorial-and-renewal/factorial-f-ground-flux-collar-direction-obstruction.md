...
Put \(h=\log2\), \(X=2m\) with \(m\ge3\), and \(U=\log(X/4)\), \(V=\log X\). Let \(E(x)=\psi(x)-x\) for right-continuous \(\psi(x)=\sum_{n\le x}\Lambda(n)\), let \(F>0\) be the actual factorial ground with \(F(x)\to\kappa_F>0\), and set \(r(u)=E(e^u)/(e^uF(e^u))\). The [full two-collar computation](../successor-and-division/successor-cell-even-odd-collar-full-matrix.md) defines the triangular profile
\[
...
The mismatch is realized by the actual \(\Lambda\) source, not just by a model error function. The [odd-collar oscillation theorem](../successor-and-division/successor-cell-odd-collar-triangular-prime-oscillation.md) gives a constant \(A_1>0\) such that
\[
...
\]
The even bracket collapses, after *all* odd Möbius divisors are summed, to exactly the powers-of-two atoms \(\Lambda(2^k)=\log2\). At most two such powers meet \((X/4,X)\), so their entire collar contribution is \(O(X^{-1/2})\). The odd bracket remains the complete odd prime-power source and carries the zeta poles: \(\zeta_{\rm odd}(s)=(1-2^{-s})\zeta(s)\), and the factor \(1-2^{-s}\) does not vanish when \(\Re s>0\). Thus the 2-adic pairing does not control the centered remainder. Classical [Vaughan's identity](https://arxiv.org/html/math/0606087#S4) and [Heath-Brown's generalized identity](https://geodesic.mathdoc.fr/articles/10.4153/CJM-1982-095-9/) repartition the same \(\mu*\log\) sum into Type I and signed bilinear ranges; they do not supply a zero-frequency sign for (16). The [existing Vaughan terminal-packet audit](../weil-and-spectral/odd-terminal-vaughan-zero-frequency-type-ii-barrier.md) exhibits the analogous large absolute Type-II mass. No new all-cutoff bound for \(D_X\) follows from (16)--(20).

[SuccessorCollarOddMobius.lean](../../formalization/BuildingBlocks/SuccessorCollarOddMobius.lean) now compiles the exact arithmetic identity behind (20), using Mathlib's actual \(\mu\), \(\Lambda\), and \(\log\). It proves \(\mu=\mu_{\rm odd}*(\delta_1-\delta_2)\), evaluates the cofactor bracket as \(\log b\) for odd \(b\) and \(\log2\) for positive even \(b\), and reindexes every finite weighted \(\Lambda\) sum into odd-divisor/cofactor pairs. The proof retains all prime powers and uses only Lean's standard foundational axioms. The analytic collar weight, Laplace transform, and all-cutoff estimate in this note remain written analysis or open targets, not formal consequences of that finite identity.
