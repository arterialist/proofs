...
The [factorial module](../../formalization/BuildingBlocks/DivisionDyadicFactorial.lean) proves the exact recurrence and strict bound for

$$
A_N=NH_N-\log((2N)!/N!).
$$

The [prime-average module](../../formalization/BuildingBlocks/DivisionDyadicPrimeAverage.lean) defines

$$
D_N=\sum_{d=1}^N\left[\psi(\lfloor2N/d\rfloor)-\psi(\lfloor N/d\rfloor)-\frac Nd\right]
$$

using the repository's actual von Mangoldt sum, and proves $D_N=-A_N$. Natural-number division in the Lean source implements these floors. It reuses the already published `BuildingBlocks.MertensTransfer.logFactorial_eq_sum_psi` in [MertensArithmetic](../../formalization/BuildingBlocks/MertensArithmetic.lean), so the integer prime-to-factorial identification is part of the formal result.
...
The divisor identities are classical Möbius inversion, specifically [DLMF 27.5.5](https://dlmf.nist.gov/27.5.E5). The written trapezoidal proof and the formal half-shift proof are retained as distinct routes to the factorial bound. Neither proves a sign for the critical two-history convolution in [the harmonic theorem](../analytic-methods/harmonic-convolution.md) or the eventual-sign premise of [the RH criterion](../zeta-and-zeros/actual-critical-sign-criterion.md).
