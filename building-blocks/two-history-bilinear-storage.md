...
The conventions agree with [the dyadic finite-band flux](prime-innovation-finite-band-flux.md). The signed scalar-phase transport in [the signed phase transport](signed-scattering-phase-transport.md) is compatible with the cancellation below, but no estimate from that note is needed for this identity.
...
The [exact arithmetic reflection formula](harmonic-convolution.md) and (9) give
