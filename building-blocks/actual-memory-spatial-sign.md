...
This written proof consumes the [compensated kernel](charge-compensated-successor-memory.md), its [zero-integral theorem](compensated-memory-integral-conservation.md), and the [arithmetic clock calibration](successor-clock-arithmetic-calibration.md). It concerns the actual causal memory commutator. No anticausal, full bilinear, sampled-birth or Weil sign is proved, and no analytic theorem here is formalized in Lean.
...
$$
 B(v)=2/v-4/v^2+O(v^{-3}),
$$
and its [calibrated constant](successor-clock-arithmetic-calibration.md) gives the primitive
$$
 \int_0^R B(v)\,dv
