# Contribution 36: Exact Finite Dyadic Divisor Renewal, Factorial Deficit Recurrence, and Unconditional Negative Prime Discrepancy
...
| **Present work (2026)** | Exact identification $D_N = -A_N$, recurrence $A_{N+1}-A_N = H_N+1-\log(4N+2)$, uniform bound $D_N < -N(3/2-\log 4)$, exact dyadic boundary cancellation in $H(\beta * v)$, and Lean 4 verification | Priority for these exact formulas and their formalization is provisional; no exhaustive search is documented. |
