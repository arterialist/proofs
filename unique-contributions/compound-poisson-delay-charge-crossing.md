...
## 5. Literature Search and Novelty Verification
...
3. **Novelty of the Result:** The deterministic negative sign of the Laplace coefficients $L_a(j) < 0$, the exact dual port balance $\delta(K a) < 0 < \delta(\mathcal{A} a)$, the entireness of $Q_{\mathrm{rel}}(t)$, the proof of the unique simple crossing time $t_*$, and the explicit non-asymptotic bounds in terms of $\eta_1$, $\kappa$, $\beta$, and $b_1$ are completely absent from prior literature.
