...
$$
\sigma\geq1-\frac{1}{48.07125(\log |t|)^{2/3}(\log\log |t|)^{1/3}}.
$$
...
The script contains 47 exact integer factors defining the normalized sum of Fejér squares

$$
P_{\mathrm{opt}}(x)=
\frac{\sum_{r=1}^{47}\left|\sum_{j=0}^{90}C_{r,j}e^{ijx}\right|^2}
{\sum_{r=1}^{47}\sum_{j=0}^{90}C_{r,j}^2}.
$$

It forms

$$
P(x)=10^{-8}P_{\mathrm{Fej\acute er}}(x)+(1-10^{-8})P_{\mathrm{opt}}(x),
$$

where $P_{\mathrm{Fej\acute er}}(x)=|\sum_{j=0}^{90}e^{ijx}|^2/91$. Both components are globally nonnegative and normalized by $b_0=1$. The Fejér component makes every cosine coefficient strictly positive. Exact rational autocorrelations then verify $b_k>0$ for every $1\leq k\leq90$ and $b_1>b_0$. Thus every polynomial admissibility condition is established without numerical sampling.

Let $b=\sum_{k=1}^{90}b_k$, and let $\theta\in(0,\pi/2)$ be the unique solution of
...
The script proves the resulting rational upper-bound comparison with $48.07125^3$ exactly. The polynomial's numerical objective is approximately $48.071245872$; this decimal is explanatory and is not used by the certificate.
