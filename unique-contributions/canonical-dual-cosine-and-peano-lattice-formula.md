...
**Primary Novelty:** First exact dual cosine and Peano kernel representations for the coherent division lattice remainder $\mathcal{Q}(t) = \sqrt{t}\sum_{m \ge 1}\log(m) L(tm)$, proving exact cancellation of the derivative boundary contact against the periodic Bernoulli term and yielding a non-negative Peano arch representation.
