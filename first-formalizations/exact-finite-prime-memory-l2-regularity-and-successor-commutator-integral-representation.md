...
This entry documents a machine-verified implementation in Lean 4 proving that the commutator of the successor operator and the finite-prime causal memory operator is an integral operator with explicit kernel $M_S(v, u)$, retaining all prime-power cells without cutoff.
