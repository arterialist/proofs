...
This formalization provides the first machine-verified proof in Lean 4 establishing the cardinal parity law $\operatorname{sgn}(\prod A(p)) = (-1)^{|B|}$ and universal non-asymptotic pinching $\frac{1}{8} p^{hM} \le -A(p) \le 2 p^{hM}$ for prime scattering multipliers.
