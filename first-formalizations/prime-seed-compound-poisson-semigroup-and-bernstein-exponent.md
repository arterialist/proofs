# Prime-seed compound Poisson semigroup and uniform Bernstein exponent
...
**Classification:** provisional first-formalization record, to the best of the targeted search. The abstract compound-Poisson and Bernstein facts are classical; the claim concerns the checked Lean package for the literal all-prime jump profile.
...
## 4. Targeted formalization search

Mathlib's probability and convolution libraries, the public KeyAIGit `ecdlp-lean-verification` repository, the public Li-criterion Lean repository, and the `PrimeNumberTheoremAnd` index were searched for a formalization combining this arithmetic prime profile with the complete positive semigroup and the uniform closed-half-plane exponent bound. No matching public Lean development was found. The existing building-block note records the classical probabilistic framework and the source-specific arithmetic construction; this page records the checked Lean scope only.

## 5. Formalization and reproducibility
